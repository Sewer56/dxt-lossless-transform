...
//!
//! This module provides BC1-specific implementations of the file format traits defined
//! in [`dxt-lossless-transform-api-common`].

use bitfield::bitfield;
use dxt_lossless_transform_api_common::embed::{
    EmbedError, EmbeddableTransformDetails, TransformFormat,
};
...
impl Bc1TransformHeaderData {
    ...
    /// Create [`Bc1TransformHeaderData`] from [`Bc1DetransformDetails`]
    ...
    /// Convert [`Bc1TransformHeaderData`] to [`Bc1DetransformDetails`]
...
/// Wrapper type for BC1 detransform details that can be stored in file headers
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct EmbeddableBc1Details(pub Bc1DetransformDetails);

impl From<Bc1DetransformDetails> for EmbeddableBc1Details {
    fn from(details: Bc1DetransformDetails) -> Self {
        Self(details)
    }
}

impl From<EmbeddableBc1Details> for Bc1DetransformDetails {
...
impl EmbeddableTransformDetails for EmbeddableBc1Details {
    ...
    fn pack(&self) -> u32 {
        Bc1TransformHeaderData::from_detransform_details(&self.0).0
    }

    fn unpack(data: u32) -> Result<Self, EmbedError> {
        Ok(Self(Bc1TransformHeaderData(data).to_detransform_details()?))
    }
}
...
mod tests {
    ...
    fn test_roundtrip_all_possible_transform_details() {
        for &variant in YCoCgVariant::all_values() {
            for &split in &[true, false] {
                ...
                let embeddable = EmbeddableBc1Details(details);
    ...
    fn test_header_version_and_reserved_fields() {
        ...
        let header = Bc1TransformHeaderData::from_detransform_details(&details);
    ...
    fn test_invalid_header_version() {
        ...
        // Should fail to convert to detransform details due to invalid version
        assert_eq!(
            invalid_header.to_detransform_details(),
