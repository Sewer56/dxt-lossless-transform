...
/// SSE2 implementation for split-colour and recorrelate untransform for BC2.
...
unsafe fn untransform_split_colour_recorr<const VARIANT: u8>(
    ...
    while output_ptr < output_end {
        ...
        // Combine color0 and color1 into packed colors for recorrelation
        let colors_0 = _mm_unpacklo_epi16(color0s, color1s);
        let colors_1 = _mm_unpackhi_epi16(color0s, color1s);

        // Apply recorrelation to the packed colors
        let recorr_colors_0 = match VARIANT {
        ...
        let recorr_colors_1 = match VARIANT {
        ...
        // Combine recorrelated colors with indices
        let colors_indices_0 = _mm_unpacklo_epi32(recorr_colors_0, indices0);
        let colors_indices_1 = _mm_unpackhi_epi32(recorr_colors_0, indices0);
        let colors_indices_2 = _mm_unpacklo_epi32(recorr_colors_1, indices1);
        let colors_indices_3 = _mm_unpackhi_epi32(recorr_colors_1, indices1);

        // Combine alpha with colors+indices to form complete blocks
...
mod tests {
    ...
    fn sse2_untransform_roundtrip(#[case] variant: YCoCgVariant) {
        if !has_sse2() {
            return;
        }

        // SSE2 processes 128 bytes per iteration (* 2 / 16 == 16)
