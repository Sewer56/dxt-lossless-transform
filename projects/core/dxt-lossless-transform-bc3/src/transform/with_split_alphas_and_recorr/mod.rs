//! # BC3 Block Splitting with Split Alpha Endpoints and YCoCg Decorrelation Module
//!
//! This module provides optimized functions for separating BC3 data into five distinct arrays
//! while applying both alpha endpoint splitting and YCoCg decorrelation to color endpoints
//! in a single optimized step. This combines the benefits of [`with_split_alphas`] and
...
//!
//! ### Decorrelated Colors Array (`decorrelated_colors_out`)
//! - Type: `*mut u32`
//! - Contains the YCoCg decorrelated color endpoints for each BC3 block (4 bytes per block)
//!
...
//! by decorrelating the color channels in the YCoCg color space. The alpha channels
//! are split into separate arrays for better compression efficiency.
//!
...
/// Transform operations that combine alpha endpoint splitting with YCoCg decorrelation
pub mod transform;
/// Untransform operations that combine alpha endpoint merging with YCoCg recorrelation
pub mod untransform;

/// Transform BC3 data from standard interleaved format to five separate arrays
/// (alpha0, alpha1, alpha_indices, decorrelated_colors, color_indices) while applying
/// YCoCg decorrelation using best known implementation for current CPU.
///
/// # Safety
///
...
/// - `decorrelated_colors_out`: Pointer to destination decorrelated colors array
...
/// BC3 blocks will have been split into five separate arrays with split alpha
/// endpoints and decorrelated colors.
///
/// # Remarks
///
...
pub(crate) unsafe fn transform_with_split_alphas_and_recorr(
    ...
    transform::transform_with_split_alphas_and_recorr(
        input_ptr,
        alpha0_out,
        alpha1_out,
        alpha_indices_out,
...
/// Transform BC3 data from five separate arrays (alpha0, alpha1, alpha_indices,
/// decorrelated_colors, color_indices) back to standard interleaved format while
/// applying YCoCg recorrelation using best known implementation for current CPU.
///
/// # Safety
///
...
/// - `decorrelated_colors_out`: Pointer to source decorrelated colors array
...
/// merged alpha endpoints and recorrelated colors.
///
/// # Remarks
///
/// This function automatically selects the best available SIMD implementation
/// for the current CPU architecture. The performance characteristics are
/// identical to [`transform_with_split_alphas_and_recorr`] but in reverse.
///
...
pub(crate) unsafe fn untransform_with_split_alphas_and_recorr(
    ...
    untransform::untransform_with_split_alphas_and_recorr(
        alpha0_out,
        alpha1_out,
        alpha_indices_out,
        decorrelated_colors_out,
