...
unsafe fn untransform_split_and_decorrelate(
    input_ptr: *const u8,
    output_ptr: *mut u8,
    len: usize,
    ...
    unsafe {
        ...
        match decorrelation_mode {
            YCoCgVariant::Variant1 => {
                untransform_split_and_decorrelate_variant::<1>(
                    colors_ptr,
                    indices_ptr,
                    output_ptr,
                    num_blocks,
            ...
            YCoCgVariant::Variant2 => {
                untransform_split_and_decorrelate_variant::<2>(
                    colors_ptr,
                    indices_ptr,
                    output_ptr,
                    num_blocks,
            ...
            YCoCgVariant::Variant3 => {
                untransform_split_and_decorrelate_variant::<3>(
                    colors_ptr,
                    indices_ptr,
                    output_ptr,
                    num_blocks,
            ...
            YCoCgVariant::None => {
                untransform_split_and_decorrelate_variant::<0>(
                    colors_ptr,
                    indices_ptr,
                    output_ptr,
                    num_blocks,
...
unsafe fn untransform_split_and_decorrelate_variant<const VARIANT: u8>(
    mut colors_ptr: *const u32,
    mut indices_ptr: *const u32,
    mut output_ptr: *mut u8,
    num_blocks: usize,
) {
    // Helper function to apply recorrelation based on variant (evaluated at compile time)
    #[inline(always)]
    fn apply_recorrelation<const V: u8>(color: Color565) -> Color565 {
        match V {
            1 => color.recorrelate_ycocg_r_var1(),
            2 => color.recorrelate_ycocg_r_var2(),
            3 => color.recorrelate_ycocg_r_var3(),
            _ => color, // No transformation for None variant
        }
    }
    ...
    unsafe {
        for _ in 0..num_blocks {
            ...
            let recorr_color0 = apply_recorrelation::<VARIANT>(color0);
            let recorr_color1 = apply_recorrelation::<VARIANT>(color1);
