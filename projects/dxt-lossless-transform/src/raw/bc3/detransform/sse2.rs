...
pub unsafe fn u64_detransform_sse2(input_ptr: *const u8, output_ptr: *mut u8, len: usize) {
    ...
    while alpha_byte_in_ptr < alpha_byte_end_ptr {
        ...
        write_u16(current_output_ptr, 0, shift_u16(alpha_bytes, 0));
        write_u16(current_output_ptr, 16, shift_u16(alpha_bytes, 16));
        write_u16(current_output_ptr, 32, shift_u16(alpha_bytes, 32));
        write_u16(current_output_ptr, 48, shift_u16(alpha_bytes, 48));

        // Handle alpha bits - read 8 bytes at a time
        let alpha_bits_0 = alpha_bit_in_ptr.read_unaligned();
        write_u16(current_output_ptr, 2, shift_u16(alpha_bits_0, 0));
        write_u32(current_output_ptr, 4, shift_u32(alpha_bits_0, 16)); // block 0 end
        write_u16(current_output_ptr, 18, shift_u16(alpha_bits_0, 48)); // block 1 start (2/6 bytes), 0 alphabytes left

        let alpha_bits_1 = alpha_bit_in_ptr.add(1).read_unaligned();
        write_u32(current_output_ptr, 20, shift_u32(alpha_bits_1, 0)); // block 1 complete (6/6 bytes), 4 alphabytes left
        write_u32(current_output_ptr, 34, shift_u32(alpha_bits_1, 32)); // block 2 start (4/6 bytes), 0 alphabytes left

        let alpha_bits_2 = alpha_bit_in_ptr.add(2).read_unaligned();
        write_u16(current_output_ptr, 38, shift_u16(alpha_bits_2, 0)); // block 2 end (6/6 bytes), 6 left
...
#[inline(always)]
unsafe fn shift_u16(value: u64, shift: usize) -> u16 {
    (value >> shift) as u16
}

#[inline(always)]
unsafe fn shift_u32(value: u64, shift: usize) -> u32 {
    (value >> shift) as u32
}
...
mod tests {
    ...
    use crate::raw::bc3::transform::u32;
    use rstest::rstest;
