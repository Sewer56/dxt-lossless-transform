...
use dxt_lossless_transform_bc2::{
    transform_bc2_with_settings, untransform_bc2_with_settings, Bc2TransformSettings,
};
...
pub(crate) fn handle_benchmark_command(cmd: BenchmarkCmd) -> Result<(), TransformError> {
    ...
    println!(
        "Benchmarking BC2 decompress+untransform performance for files in: {} (recursive)",
        input_path.display()
    );
...
fn process_scenario(
    data: &[u8],
    scenario_name: &str,
    transform_details: Bc2TransformSettings,
    config: &BenchmarkConfig,
    caches: &CacheRefs,
