...
use dxt_lossless_transform_bc2::transform_bc2_auto;
...
/// Debug commands for analyzing BC2 files
#[argh(subcommand, name = "debug-bc2")]
pub struct DebugCmd {
    #[argh(subcommand)]
...
#[derive(FromArgs, Debug)]
/// Test BC2 transform/untransform roundtrip on files in a directory
...
#[derive(FromArgs, Debug)]
/// Compress BC2 files with all transform combinations and collect compression statistics
...
#[derive(FromArgs, Debug)]
/// Benchmark BC2 transform and untransform performance on files in a directory
...
#[derive(FromArgs, Debug)]
/// Benchmark BC2 determine_best_transform_details function performance on files in a directory
...
/// # Parameters
///
/// - `data`: Slice containing the BC2 data
...
) -> Result<dxt_lossless_transform_bc2::Bc2TransformSettings, TransformError>
where
    ...
    T::Error: core::fmt::Debug,
{
    ...
    let transform_options = Bc2EstimateSettings {
        size_estimator,
    ...
    unsafe {
        // Use standard API (BC2 doesn't have experimental normalization)
        transform_bc2_auto(data_ptr, output_ptr, len_bytes, &transform_options)
