...
pub(crate) fn handle_roundtrip_command(cmd: RoundtripCmd) -> Result<(), TransformError> {
    ...
    println!(
        "Testing BC2 transform/untransform roundtrip on files in: {} (recursive)",
        input_path.display()
    );
    ...
    entries.par_iter().for_each(|entry| {
        ...
        match test_bc2_roundtrip_file(entry) {
            Ok(()) => {
...
fn test_bc2_roundtrip(data: &[u8]) -> Result<(), TransformError> {
    use dxt_lossless_transform_bc2::{
        transform_bc2_with_settings, untransform_bc2_with_settings, util::decode_bc2_block,
    };
...
mod tests {
    ...
    #[test]
    fn test_bc2_roundtrip_on_test_file() {
        ...
        let result = test_bc2_roundtrip_file(&dir_entry);

        // Assert the test passes
        assert!(result.is_ok(), "BC2 roundtrip test failed: {result:?}");
    }
}
