...
use dxt_lossless_transform_bc3::{transform_bc3_with_settings, Bc3TransformSettings};
...
// Type aliases for BC3-specific compression stats
type Bc3CompressionStatsResult =
    calc_compression_stats_common::CompressionStatsResult<Bc3TransformSettings>;
type Bc3TransformResult = calc_compression_stats_common::TransformResult<Bc3TransformSettings>;
...
pub(crate) fn handle_compression_stats_command(
    ...
    println!(
        "Analyzing BC3 compression statistics for files in: {} (recursive)",
        input_path.display()
    );
    println!(
    ...
    let results = Mutex::new(Vec::<Bc3CompressionStatsResult>::new());
    ...
    entries
        ...
        .with_max_len(1)
        .for_each(|entry| {
            match analyze_bc3_compression_file(
                entry,
                cmd.get_compression_level(),
                cmd.get_estimate_compression_level(),
...
fn analyze_bc3_compression_file(
    entry: &fs::DirEntry,
...
) -> Result<Bc3CompressionStatsResult, TransformError> {
    let mut file_result: Bc3CompressionStatsResult = Bc3CompressionStatsResult::default();

    extract_blocks_from_file(
        &entry.path(),
        TransformFormatFilter::Bc3,
        |data: &[u8], _format: TransformFormat| -> Result<(), TransformError> {
            file_result = Bc3CompressionStatsResult {
                file_path: entry.path().display().to_string(),
                original_uncompressed_size: data.len(),
                all_results: analyze_bc3_compression_transforms(
                    data,
                    compression_level,
                ...
                api_recommended_result: analyze_bc3_api_recommendation(
                    data,
...
fn analyze_bc3_compression_transforms(
    data: &[u8],
    compression_level: i32,
    compression_algorithm: CompressionAlgorithm,
    cache: &Mutex<CompressionCache>,
) -> Result<Vec<Bc3TransformResult>, TransformError> {
    ...
    unsafe {
        ...
        for transform_options in Bc3TransformSettings::all_combinations() {
            // Transform the data
            transform_bc3_with_settings(
                data.as_ptr(),
            ...
            results.push(Bc3TransformResult {
                transform_options,
...
fn analyze_bc3_api_recommendation(
    data: &[u8],
    estimate_compression_level: i32,
    estimate_compression_algorithm: CompressionAlgorithm,
    ...
    use_all_decorrelation_modes: bool,
    cache: &Mutex<CompressionCache>,
) -> Result<Bc3TransformResult, TransformError> {
    ...
    Ok(Bc3TransformResult {
        transform_options: best_details,
        compressed_size,
    })
}

/// Formats [`Bc3TransformSettings`] as a human-readable string
fn format_transform_details(details: Bc3TransformSettings) -> String {
    ...
    let split_alpha = if details.split_alpha_endpoints {
        "SplitAlpha"
    } else {
        "NoSplitAlpha"
    };

    let split_colour = if details.split_colour_endpoints {
        "SplitColour"
    } else {
        "NoSplitColour"
    };

    format!("{decorr_mode}/{split_alpha}/{split_colour}")
}
