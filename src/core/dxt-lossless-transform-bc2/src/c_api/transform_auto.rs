//! BC2 automatic transform operations for C API.
//!
//! This module provides C-compatible FFI functions for transforming BC2 data
//! using automatically determined optimal settings.

use crate::{transform_bc2_auto_safe, Bc2AutoTransformError, Bc2EstimateSettings};
use core::slice;
use dxt_lossless_transform_api_common::c_api::size_estimation::DltSizeEstimator;
use dxt_lossless_transform_common::color_565::YCoCgVariant;

/// Settings for automatic BC2 transform configuration.
///
...
pub struct Dltbc2AutoTransformSettings {
...
/// Error codes for BC2 operations.
#[repr(C)]
...
/// Result type for BC2 C API operations.
#[repr(C)]
...
impl Dltbc2Result {
    ...
    pub fn from_error_code(error_code: Dltbc2ErrorCode) -> Self {
        Self { error_code }
    }
...
impl<T: core::fmt::Debug> From<Bc2AutoTransformError<T>> for Dltbc2Result {
    fn from(error: Bc2AutoTransformError<T>) -> Self {
        let error_code = match error {
            Bc2AutoTransformError::InvalidLength(_) => Dltbc2ErrorCode::InvalidDataLength,
            Bc2AutoTransformError::OutputBufferTooSmall { .. } => {
                Dltbc2ErrorCode::OutputBufferTooSmall
            }
            Bc2AutoTransformError::DetermineBestTransform(_) => {
                Dltbc2ErrorCode::SizeEstimationError
            }
        };
...
impl From<crate::Bc2TransformSettings> for Dltbc2TransformSettings {
    fn from(settings: crate::Bc2TransformSettings) -> Self {
        Self {
...
impl From<Dltbc2TransformSettings> for crate::Bc2TransformSettings {
    fn from(settings: Dltbc2TransformSettings) -> Self {
        Self {
            split_colour_endpoints: settings.split_colour_endpoints,
            decorrelation_mode: settings.decorrelation_mode,
        }
...
/// - `out_details` must be a valid pointer for writing [`Dltbc2TransformSettings`]
/// - The estimator's context and functions must remain valid for the duration of the call
#[unsafe(no_mangle)]
pub unsafe extern "C" fn dltbc2core_transform_auto(
    data: *const u8,
    data_len: usize,
    output: *mut u8,
    output_len: usize,
    estimator: *const DltSizeEstimator,
    settings: Dltbc2AutoTransformSettings,
    out_details: *mut Dltbc2TransformSettings,
) -> Dltbc2Result {
    // Validate pointers
    if data.is_null() {
    ...
    if estimator.is_null() {
        return Dltbc2Result::from_error_code(Dltbc2ErrorCode::NullEstimatorPointer);
    }
    if out_details.is_null() {
        return Dltbc2Result::from_error_code(Dltbc2ErrorCode::NullTransformSettingsPointer);
    }
    ...
    match transform_bc2_auto_safe(data_slice, output_slice, &options) {
...
mod tests {
    ...
    #[test]
    fn test_dltbc2core_transform_auto_null_data() {
        let estimator = create_dummy_estimator();
    ...
    #[test]
    fn test_dltbc2core_transform_auto_null_output() {
        let estimator = create_dummy_estimator();
        let test_data = create_test_bc2_data();
        let mut out_details = Dltbc2TransformSettings {
            split_colour_endpoints: false,
    ...
    #[test]
    fn test_dltbc2core_transform_auto_null_estimator() {
        let test_data = create_test_bc2_data();
        let mut output = vec![0u8; test_data.len()];
        let mut out_details = Dltbc2TransformSettings {
            split_colour_endpoints: false,
        ...
        unsafe {
            ...
            assert_eq!(result.error_code, Dltbc2ErrorCode::NullEstimatorPointer);
            assert!(!result.is_success());
        }
    }

    #[test]
    fn test_dltbc2core_transform_auto_null_out_details() {
        let estimator = create_dummy_estimator();
        let test_data = create_test_bc2_data();
        let mut output = vec![0u8; test_data.len()];
        let settings = Dltbc2AutoTransformSettings {
    ...
    #[test]
    fn test_dltbc2core_transform_auto_invalid_length() {
        let estimator = create_dummy_estimator();
    ...
    #[test]
    fn test_dltbc2core_transform_auto_output_too_small() {
        let estimator = create_dummy_estimator();
        let test_data = create_test_bc2_data();
    ...
    #[test]
    fn test_dltbc2core_transform_auto_different_modes() {
        let estimator = create_dummy_estimator();
        let test_data = create_test_bc2_data();
    ...
    #[test]
    fn test_dltbc2_result_success() {
        let result = Dltbc2Result::success();
        assert_eq!(result.error_code, Dltbc2ErrorCode::Success);
        assert!(result.is_success());
    }

    #[test]
    fn test_dltbc2_result_from_error_code() {
        let result = Dltbc2Result::from_error_code(Dltbc2ErrorCode::InvalidDataLength);
        assert_eq!(result.error_code, Dltbc2ErrorCode::InvalidDataLength);
        assert!(!result.is_success());
    }

    #[test]
    fn test_dltbc2_transform_settings_conversion() {
    ...
    #[test]
    fn test_dltbc2_transform_settings_from_rust() {
        let rust_settings = crate::Bc2TransformSettings {
            split_colour_endpoints: false,
            decorrelation_mode: YCoCgVariant::Variant2,
        };

        let c_settings: Dltbc2TransformSettings = rust_settings.into();
