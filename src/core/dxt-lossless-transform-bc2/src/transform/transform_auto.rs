//! BC2 Transform Optimization
//!
//! This module provides optimization functionality to determine the best
//! transformation parameters for BC2 data compression.

use dxt_lossless_transform_api_common::estimate::SizeEstimationOperations;
use dxt_lossless_transform_common::allocate::{allocate_align_64, AllocateError};
use thiserror::Error;

use super::settings::{Bc2TransformSettings, COMPREHENSIVE_TEST_ORDER, FAST_TEST_ORDER};
use super::transform_with_settings::transform_bc2_with_settings;
...
/// The settings for [`transform_bc2_auto`], regarding how the estimation is done,
/// and other related factors.
pub struct Bc2EstimateSettings<T>
where
    T: SizeEstimationOperations,
{
    ...
    /// maximize speed of [`transform_bc2_auto`], and to improve decompression speed
...
/// Transform BC2 data using the best determined settings.
///
...
/// # Returns
///
/// The [`Bc2TransformSettings`] that produced the best (smallest) compressed size.
///
/// # Remarks
///
...
/// based on analysis of BC1 texture files (similar patterns expected for BC2):
...
///
/// ```rust,no_run
/// # use dxt_lossless_transform_bc2::{transform_bc2_auto, Bc2EstimateSettings};
/// # use dxt_lossless_transform_api_common::estimate::SizeEstimationOperations;
///
...
/// }
///
/// let bc2_data = vec![0u8; 16]; // Example BC2 block data
/// let mut output_buffer = vec![0u8; bc2_data.len()]; // Output buffer
...
///
/// Determines the best [`Bc2TransformSettings`] by brute force testing of different transformation
...
///
/// - Alpha data and index data are excluded from size estimation as they have poor compressibility
...
pub unsafe fn transform_bc2_auto<T>(
    input_ptr: *const u8,
    output_ptr: *mut u8,
    len: usize,
    transform_options: &Bc2EstimateSettings<T>,
) -> Result<Bc2TransformSettings, DetermineBestTransformError<T::Error>>
where
    T: SizeEstimationOperations,
{
    let mut best_transform_settings = Bc2TransformSettings::default();
    let mut best_size = usize::MAX;
    let mut last_tested = Bc2TransformSettings::default();

    // Pre-allocate compression buffer once for all iterations
    // Note: len/4 because we only compress color data from BC2 (4 bytes color endpoints out of 16 bytes total)
    ...
    for &(decorrelation_mode, split_colours) in test_order {
        ...
        let current_mode = Bc2TransformSettings {
            decorrelation_mode,
            split_colour_endpoints: split_colours,
        };

        // Apply a full transformation (~24GB/s on 1 thread, Ryzen 9950X3D)
        transform_bc2_with_settings(input_ptr, output_ptr, len, current_mode);
        last_tested = current_mode;

        // Note: The alpha and indices are very poorly compressible (entropy == ~7.0 , no lz matches).
        ...
        // For BC2, we estimate only the color portion (similar to BC1 approach)
        let result_size = transform_options
            .size_estimator
            .estimate_compressed_size(
                output_ptr.add(len / 2), // skip alpha data, proceed to color endpoints
                len / 4, // color endpoints only. Indices don't compress well, alphas are not really transformed.
                comp_buffer_ptr,
                comp_buffer_len,
            )
            .map_err(DetermineBestTransformError::SizeEstimationError)?;
    ...
    if best_transform_settings != last_tested {
        ...
        transform_bc2_with_settings(input_ptr, output_ptr, len, best_transform_settings);
    }
