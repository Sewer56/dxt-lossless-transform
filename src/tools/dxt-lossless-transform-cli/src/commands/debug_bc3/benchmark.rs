...
use dxt_lossless_transform_bc3::{
    transform_bc3_with_settings, untransform_bc3_with_settings, Bc3TransformSettings,
};
...
pub(crate) fn handle_benchmark_command(cmd: BenchmarkCmd) -> Result<(), TransformError> {
    ...
    println!(
        "Benchmarking BC3 decompress+untransform performance for files in: {} (recursive)",
        input_path.display()
    );
...
fn process_scenario(
    data: &[u8],
    scenario_name: &str,
    transform_details: Bc3TransformSettings,
    config: &BenchmarkConfig,
    caches: &CacheRefs,
