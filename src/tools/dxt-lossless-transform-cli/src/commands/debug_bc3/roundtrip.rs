...
pub(crate) fn handle_roundtrip_command(cmd: RoundtripCmd) -> Result<(), TransformError> {
    ...
    println!(
        "Testing BC3 transform/untransform roundtrip on files in: {} (recursive)",
        input_path.display()
    );
    ...
    entries.par_iter().for_each(|entry| {
        ...
        match test_bc3_roundtrip_file(entry) {
            Ok(()) => {
...
fn test_bc3_roundtrip(data: &[u8]) -> Result<(), TransformError> {
    use dxt_lossless_transform_bc3::{
        transform_bc3_with_settings, untransform_bc3_with_settings, util::decode_bc3_block,
    };
    ...
    for transform_options in Bc3TransformSettings::all_combinations() {
        ...
        for block_idx in 0..num_blocks {
            ...
            if original_decoded != roundtrip_decoded {
                ...
                let split_alpha_endpoints = if transform_options.split_alpha_endpoints {
                    "SplitAlpha"
                } else {
                    "NoSplitAlpha"
                };

                let split_colour_endpoints = if transform_options.split_colour_endpoints {
                    "SplitColour"
                } else {
                    "NoSplitColour"
                };
                ...
                return Err(TransformError::Debug(format!(
                    "Pixel mismatch in block {block_idx} (byte offset {block_offset}) for transform {decorr_mode}/{split_alpha_endpoints}/{split_colour_endpoints}. Transform/untransform is not lossless!"
                )));
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_bc3_roundtrip_on_test_file() {
        let test_file_path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()
            .unwrap()
            .parent()
            .unwrap()
            .join("assets/tests/r2-256-bc3.dds");
        ...
        let result = test_bc3_roundtrip_file(&dir_entry);

        // Assert the test passes
        assert!(result.is_ok(), "BC3 roundtrip test failed: {result:?}");
    }
}
